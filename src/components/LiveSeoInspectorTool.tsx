      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-white/8">
        <div>
          <div className="flex items-center space-x-2">
            <Terminal className="w-4 h-4 text-[#B7FF3C]" />
            <span className="text-xs font-mono uppercase tracking-wider text-[#B7FF3C] font-bold">
      ...
      <form onSubmit={handleScan} className="mt-6 grid grid-cols-1 sm:grid-cols-12 gap-3">
        <div className="sm:col-span-6">
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl bg-[#161C28] border border-white/10 text-white placeholder-[#7D8595] text-sm focus:outline-none focus:border-[#B7FF3C] transition-colors"
            required
          />
        </div>
        ...
        <div className="sm:col-span-2 flex items-end">
          <button
            type="submit"
            disabled={isScanning}
            className="w-full py-3 rounded-xl bg-[#B7FF3C] text-[#080A0F] font-bold text-sm hover:bg-[#A8F536] transition-all flex items-center justify-center space-x-1.5 shadow-[0_0_20px_-3px_rgba(183,255,60,0.3)] disabled:opacity-50"
          >
            {isScanning ? (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-[#0D1017] border border-white/8">
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right">
                ...
                <div className="text-lg font-bold text-[#B7FF3C] font-mono">{scanResult.score}/100</div>
              </div>
              <button
                onClick={() => onRequestReview(scanResult.domain)}
                className="px-4 py-2 rounded-xl bg-[#B7FF3C] text-[#080A0F] text-xs font-bold hover:bg-[#A8F536] transition-colors flex items-center space-x-1"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {scanResult.checks.map((c: any, i: number) => (
              <div key={i} className="p-3.5 rounded-xl bg-[#161C28] border border-white/5 flex items-start space-x-3">
                {c.status === 'pass' && <CheckCircle2 className="w-4 h-4 text-[#B7FF3C] flex-shrink-0 mt-0.5" />}
