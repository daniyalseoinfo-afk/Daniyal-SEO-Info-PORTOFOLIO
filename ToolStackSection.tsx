      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {TOOL_STACK.map((tool) => (
            <div
              key={tool.name}
              className="p-6 rounded-3xl bg-[#11151E] border border-white/8 hover:border-[#2DD4BF]/40 transition-all group flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between mb-4">
                  <span className="text-xs font-mono px-2.5 py-1 rounded-md bg-[#161C28] text-[#2DD4BF] border border-white/5">
                    {tool.tag}
                  </span>
                ...
                <ul className="space-y-2 text-xs text-[#B5BBC7]">
                  {tool.features.map((feat, i) => (
                    <li key={i} className="flex items-start space-x-2">
                      <span className="text-[#2DD4BF] font-mono font-bold mt-0.5">•</span>
                      <span>{feat}</span>
                    </li>
                  ))}
                </ul>
...
export const AnalyticsPhilosophySection: React.FC = () => {
  ...
  return (
    <section id="analytics-philosophy" className="py-24 sm:py-32 bg-[#080A0F] relative overflow-hidden">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-full max-w-4xl h-96 bg-[#2DD4BF]/6 rounded-full blur-3xl pointer-events-none -z-10" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mb-12">
          ...
          <h2 className="text-3xl sm:text-5xl font-extrabold text-[#F7F8FA] tracking-tight font-display">
            Rankings are useful.{' '}
            <span className="text-[#2DD4BF]">Business impact</span> matters more.
          </h2>
          <p className="mt-4 text-base sm:text-lg text-[#B5BBC7]">
        ...
        <div className="rounded-3xl bg-[#11151E] border border-white/12 p-6 sm:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.7)] relative">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-white/8">
            ...
            <div className="flex items-center space-x-2 bg-[#161C28] p-1.5 rounded-xl border border-white/8">
              {[
                { id: 'clicks', label: 'Organic Clicks', color: '#2DD4BF' },
              ...
              {[
                ...
                return (
                  <div key={col.month} className="flex-1 flex flex-col items-center group relative z-10">
                    ...
                    {/* Tooltip on hover */}
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-12 px-2.5 py-1 rounded-md bg-[#161C28] border border-white/15 text-[10px] font-mono text-[#2DD4BF] whitespace-nowrap shadow-xl">
                      {col.stage}
                    </div>

                    {/* Bar */}
                    <div className="w-full max-w-[48px] bg-[#161C28] rounded-t-xl overflow-hidden h-48 flex items-end border border-white/5 group-hover:border-[#2DD4BF]/50 transition-colors">
                      <div
          ...
          <div className="pt-4 border-t border-white/8 flex flex-col sm:flex-row items-center justify-between text-xs font-mono text-[#7D8595] gap-2">
            ...
            <span className="text-[#2DD4BF] font-semibold">Zero Fake Metrics Guarantee</span>
          </div>
